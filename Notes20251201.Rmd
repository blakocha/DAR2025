...
I am downloading data on the [income quintile share ratio](https://ec.europa.eu/eurostat/databrowser/product/page/ilc_di11c) and [Gini coefficient](https://ec.europa.eu/eurostat/databrowser/view/ilc_di12/default/table?lang=en) from the eurostat using `eurostat` package. 

I am filtering the data to include the data for the total population (no sex/age groups etc.) and then merging the two data frames. 

```{r warning=FALSE, message=FALSE}
library(eurostat)
DatasetG <- get_eurostat("ilc_di11c")
DataGini <- get_eurostat("ilc_di12")
d1 <- DatasetG[DatasetG$age=="TOTAL" & DatasetG$sex=="T" ,c("geo", "TIME_PERIOD", "values")]
names(d1) <- c("geo", "TIME_PERIOD", "qratio")
d2 <- DataGini[DataGini$age=="TOTAL",c("geo", "TIME_PERIOD", "values")]
names(d2) <- c("geo", "TIME_PERIOD", "gini")
merge1 <- merge(d1, d2)
head(merge1, 10)
merge2 <- merge(d1, d2, all=TRUE)
head(merge2, 10)
```

Notice what was changes after including the `all=TRUE` option. 
...
Filtering the rows with the `filter` command:
...
mtcars %>%
  select(wt, hp) %>%
  filter((wt>4 | wt<2) & (hp>200| hp<150))
```
...
Run `head()` to see just a first few rows in the console. 
...
I can see that in my table there are `r dim(my_data)[1]` observations and `r dim(my_data)[2]` variables. 
