...
$H_0$: There is no dependency between gender and eye colour. 

$H_A$: There is dependency between gender and eye colour. 

Chi-square test. 

```{r}
chisq.test(tab)
chisq.test(table(d$gender, d$eye_colour2))
```

## Bootstrapping

Universal simulation-based tool to obtain the confidence interval for a population parameter from a sample. 

Let's construct a 95-percent CI (confidence interval) for the mean height in population with bootstrapping.

We have a sample of n=52 and we create 10000 of size n=52 (these are bootstrap samples). In each bootstrap sample we compute the mean. We have a distribution of those means and we can get a confidence interval from this distribution.

```{r}
orig <- d$height
nsim <- 1e4
results <- replicate(nsim, 
                     {boot <- sample(orig, length(orig), replace = TRUE)
                      mean(boot)}
                     )
hist(results, breaks=100)
```

We can get the confidence interval for the mean using the quantiles of the simulated distribution. For the 95-percent distribution you take quantiles 2.5% (0,025) and 100%-2.5% (0.975). 
