...
### Age guessing exercise:

Age guessing:
https://forms.gle/eQ5ZUqXr2i9eacpD7

Age guessing results:
https://docs.google.com/spreadsheets/d/1CCC_-5LSy8_3nQYi55-dtQKhkbgcstzul8LNt78WVNU/edi
t?usp=sharing

Download the results to R:

```{r}
URL = "https://docs.google.com/spreadsheets/d/1CCC_-5LSy8_3nQYi55-dtQKhkbgcstzul8LNt78WVNU/edit?usp=sharing"
library(googlesheets4)
gs4_deauth()
age <- read_sheet(URL)
```
...
for(i in 2:14){
  print("===================================")
  print(age$id[i])
  print(age[i,2:11]-correct)
  print("sum of absolute differences: ")
  print(sum(abs(age[i,2:11]-correct)))}
```

For which face there was the lowest dispersion in answers?

```{r}
for(column_number in 2:11){
  print(names(age)[column_number])
  print(mean(age[[column_number]]))
  print(sd(age[[column_number]]))
}
```

### Distribution shape measures

More here: https://bookdown.org/blazej_kochanski/statistics1/shape.html

Skewness measures the symmetry:

```{r}
#install.packages("e1071")
library(e1071)
skewness(d$head_circ, na.rm=TRUE)
skewness(d$fb_friends, na.rm=TRUE)
```

Kurtosis measures (the extremity of) the tails

```{r}
library(e1071)
kurtosis(d$head_circ, na.rm=TRUE)
kurtosis(d$fb_friends, na.rm=TRUE)
```
